...
func _card_array_isfull_helper() -> bool:
	if(Global.PlayerVariables["curr_clicks"] <= 0):
		return true
	else:
		return false
...
func _rearrange_helper(card : Card2D, curr_card_index : int) -> void:
	var middle_card_index : float = ((Global.PlayerVariables["max_clicks"] - 1) / 2.0) # get index of middle card (or index between middle two cards)
