...
## {OVERRIDE}
const QUEUE_HIGHLIGHT_FACTOR = 1
...
func _card_removal_unique(_card_array_position : int, card : CardUI):
	card.need_discard.connect(_request_discard_handler)
	card.card_effect.connect(_card_effect_handler)
...
## {OVERRIDE}
var _CARD_BUFFER := Vector2(500, 0) ## distance cards should be apart from eachother in hand (in pixels)
func _rearrange_helper(card : CardUI, curr_card_index : int) -> void:
	...
	card.move_and_scale(Vector2(dist_from_center * _CARD_BUFFER.x / _card_array.size(),0))
	card.rotation = 0


#TODO: not this
func _request_discard_handler(card : CardUI):
