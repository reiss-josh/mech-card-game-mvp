...
@onready var _last_rotation = self.rotation
@onready var _last_position = self.position
@onready var _last_scale = self.scale
...
func start_highlight() -> void:
	...
	_last_position = self.position
	_last_scale = self.scale
	#set new visuals
	var new_rotation = 0
	var new_scale = self.scale * _highlight_transform_scale_offset
	var new_position = Vector2(self.position.x, _highlight_transform_y_offset)
	_update_appearance(new_rotation, new_position, new_scale, _HIGHLIGHT_Z_INDEX)
...
func end_highlight() -> void:
	...
	#reset visuals
	_update_appearance(_last_rotation, _last_position, _last_scale, _last_z_index)


## Updates appearance to match [new_transform] and [new_z_index]
#(helper for highlight functions above)
func _update_appearance(rot : float, pos : Vector2, scl : Vector2, z_ind : int) -> void:
	self.rotation = rot
	self.position = pos
	self.scale = scl
	self.z_index = z_ind
